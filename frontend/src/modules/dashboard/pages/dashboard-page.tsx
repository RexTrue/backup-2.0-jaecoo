...
export function DashboardPage() {
  ...
  const userItems = useMemo(() => mergeEntities(usersQuery.data ?? [], localUsers, (item) => item.id_user || item.email), [localUsers, usersQuery.data]);
  ...
  const config = useMemo<DashboardConfig>(() => {
    ...
    const serviceByWo = new Map(serviceItems.map((item) => [item.id_wo, item]));
    nextConfig.activeList = workOrderItems
      .map((item) => {
        const linkedService = serviceByWo.get(item.id_wo);
        const href = linkedService ? `/services/${linkedService.id_servis}` : `/work-orders?search=${encodeURIComponent(formatWorkOrderCode(item))}`;
        return {
          wo: formatWorkOrderCode(item),
          plate: item.no_rangka,
          model: linkedService?.keluhan ? 'Terhubung dari WO' : 'Belum ada detail servis',
          status: linkedService?.status ?? 'ANTRIAN',
          time: item.waktuMasuk?.slice(11, 16) ?? '--:--',
          href,
          isNew: hasAnyUnseen(role, item.id_wo),
        };
      })
      .filter((item) => {
        if (role === 'FRONTLINE') return ['ANTRIAN', 'SELESAI', 'TERKENDALA', 'DIKERJAKAN'].includes(item.status);
  ...
  return (
    <div className="space-y-5">
      <DashboardHeroSection config={config} services={serviceItems} />
