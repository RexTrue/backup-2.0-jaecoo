...
export function UserCreatePage() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      appendLocalEntity('users', localRecord);
      showToast({ title: 'Pegawai ditambahkan', description: 'Akun baru akan muncul pada daftar pegawai.', tone: 'success' });
      navigate('/users');
    } catch {
      appendLocalEntity('users', localRecord);
      setSubmitState('Pegawai disimpan ke mode lokal sambil menunggu backend aktif.');
      showToast({ title: 'Tersimpan di mode lokal', description: 'Flow tambah pegawai tetap bisa diuji.', tone: 'info' });
      navigate('/users');
    }
  });
