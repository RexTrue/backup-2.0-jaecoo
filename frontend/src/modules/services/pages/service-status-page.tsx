...
export function ServiceStatusPage() {
  ...
  const records = useMemo(() => {
    const allRecords = buildWorkOrderRecords({ workOrders, services, vehicles: localVehicles, customers: localCustomers });
  ...
  }, [currentStatus, localCustomers, localVehicles, search, services, sortBy, workOrders]);
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        eyebrow="Servis"
        title={serviceStatusLabelMap[currentStatus]}
        actions={<Link to="/services"><Button variant="secondary" type="button">Kembali ke Board</Button></Link>}
      />
      ...
      <div className="grid gap-3 lg:grid-cols-[1fr_180px]">
        <Input
          ...
          placeholder="Cari WO, nama pemilik, plat, HP, model, status..."
        />
            ...
            {pagedRecords.map((record) => (
              <Card key={`${record.workOrder.id_wo}-${record.service?.id_servis ?? 'noservice'}`} className={`space-y-4 border ${serviceStatusPanelMap[currentStatus]}`}>
                ...
                <div className="flex flex-wrap gap-3">
                  {record.service ? (
                    <Link to={`/services/${record.service.id_servis}`}><Button variant="secondary" type="button">Detail Servis</Button></Link>
                  ) : null}
